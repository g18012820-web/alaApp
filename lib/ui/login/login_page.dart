...
class _LoginPageState extends FoundationState<LoginPage, LoginBloc> {
  ...
  /// الدخول الموحّد الذكي (§12.1): نفس الشاشة والحقول لكل من المالك والطالب.
  /// إن طابقت البيانات المُدخلة بيانات المالك المحفوظة محليًا (راجع
  /// `OwnerAuthStore`) يُوجَّه تلقائيًا للوحة تحكم المالك، وإلا يُعامَل كطالب.
  ///
  /// ⚠️ لا يوجد خادم مصادقة حقيقي بعد؛ أي بريد/كلمة مرور غير فارغين تُقبل
  /// كدخول طالب تجريبي (راجع `docs/owner-login-integration.md`).
  Future<void> _handleSubmit() async {
    final email = _emailController.text.trim();
    final password = _passwordController.text;

    if (email.isEmpty || password.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('الرجاء إدخال البريد الإلكتروني وكلمة المرور')));
      return;
    }

    setState(() => _submitting = true);
    final isOwner = await OwnerAuthStore.matches(email: email, password: password);
    if (!mounted) return;
    setState(() => _submitting = false);

    if (isOwner) {
      AutoRouter.of(context).replaceAll([const OwnerDashboardRoute()]);
    } else {
      final studentId = OwnerContentRepository.instance.findOrCreateByEmail(email: email);
