...
class _CourseDetailPageState extends FoundationState<CourseDetailPage, CourseDetailBloc> {
        ...
        builder: (context, state) {
          return CommonScaffold(
            ...
            bottomNavigationBar: CommonBottomNavigatorBarBackground(
              visibleBorder: false,
              child: CommonButton(
                onPressed: () => _handleEnroll(context, state.course),
  ...
  Future<void> _handleEnroll(BuildContext context, CourseEntity course) async {
    final studentId = await CurrentStudentSession.getCurrentStudentId();
    if (studentId == null) {
      if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('سجّل الدخول أولاً للشراء')));
      return;
    }
    if (OwnerContentRepository.instance.isEnrolled(studentId, course.id)) {
      if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('أنت مسجّل في هذه الدورة بالفعل')));
      return;
    }
        ...
        return SafeArea(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                Text('طريقة التسجيل في الدورة', style: AppTextStyles.h5Bold),
                const Gap(16),
                ListTile(
                  leading: const Icon(Icons.account_balance_wallet_outlined),
                  title: const Text('الشراء بالمحفظة'),
                  subtitle: Text('${course.displayPrice()}'),
                  onTap: () {
                    Navigator.pop(sheetContext);
                    final success = WalletRepository.instance.purchaseWithBalance(
                      studentId: studentId,
                      courseId: course.id,
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text(success ? 'تم التسجيل في الدورة بنجاح 🎉' : 'رصيدك غير كافٍ، اشحن محفظتك أولاً')),
                    );
                  },
                ),
                ListTile(
                  leading: const Icon(Icons.confirmation_number_outlined),
                  title: const Text('فتح بالكود'),
                  onTap: () {
                    Navigator.pop(sheetContext);
                    AutoRouter.of(context).push(RedeemCodeRoute(studentId: studentId));
                  },
                ),
              ],
            ),
